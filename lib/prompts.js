...
- Fire perimeter coordinates: ${JSON.stringify(scenario.damage.fire_perimeter.slice(0, 20))}
...
Generate exactly 5 DIVERSE disaster recovery plans as a JSON array. Each plan must have this exact structure:
{
  "plan_id": <number 1-5>,
...
Vary strategies: one prioritizing containment, one prioritizing evacuation, one balanced, one protecting infrastructure, one rapid distributed response.
...
Generate 5 NEW plans that IMPROVE on the best plan. Try:
...
Each plan must follow the same JSON schema as before. All coordinates must be within the county.
